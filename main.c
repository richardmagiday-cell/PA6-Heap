...
int compareTo(Cat *a, Cat *b, Mode mode) {
    ...
    if (a->arrival_id < b->arrival_id) return  1;
    return -1;
}
...
// Sets the shelter mode, syncs the heap's comparator, then rebuilds all priorities.
void cmd_mode(Shelter *S, const char *mode_str) {
...
int main(void) {
    ...
    free(S.heap.arr);
    if (S.featured_breed != NULL) free(S.featured_breed);
